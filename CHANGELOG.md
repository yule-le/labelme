...
- Fixed AI Assist interactions appearing to return nothing when every predicted Shape matched an existing Shape. Matching existing Shapes are now highlighted instead of duplicated or silently suppressed, while unmatched predictions are still added normally. Point prompts choose the candidate that best satisfies their positive and negative points ([#2444](https://github.com/wkentaro/labelme/pull/2444))
