...
- Fixed AI Assist point prompts being silently suppressed whenever their single returned Shape overlapped an existing Shape, which made nested and overlapping objects impossible to annotate and hid the hover preview over those regions. A direct answer now bypasses existing-Shape duplicate suppression, while multi-result Sweeps continue to suppress duplicates based on their raw response count ([#2444](https://github.com/wkentaro/labelme/pull/2444))
